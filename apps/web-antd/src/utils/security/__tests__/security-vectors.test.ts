...
describe('security vectors', () => {
  ...
  it('signs the complete admin role assignment request', () => {
  ...
  it('signs role definition and permission assignment fields', () => {
    expect(resolvePolicyForAlias('role.add').requestSign).toEqual([
      'title',
      'pid',
      'status',
    ]);
    expect(resolvePolicyForAlias('role.update').requestSign).toEqual([
      'title',
      'pid',
    ]);
    expect(resolvePolicyForAlias('role.status.update').requestSign).toEqual([
      'status',
    ]);
    expect(resolvePolicyForAlias('role.permission.update').requestSign).toEqual(
      ['routePermissionIds', 'docPermissionIds'],
    );
  });

  it('signs route permission definition fields', () => {
    expect(resolvePolicyForAlias('permission.add').requestSign).toEqual([
      'uuid',
      'title',
      'module',
      'pid',
    ...
    expect(resolvePolicyForAlias('permission.update').requestSign).toEqual([
      'uuid',
      'title',
      'module',
      'pid',
    ...
    expect(
      resolvePolicyForAlias('permission.status.update').requestSign,
    ).toEqual(['status']);
  });
  ...
  it('signs a request with base headers when business fields are empty', async () => {
    const key = '1234567890abcdef';
    const iv = 'abcdef1234567890';
    const traceID = 'trace-demo-000';
    const timestamp = '1700000000';
    vi.stubEnv('VITE_ADMIN_SECURITY_APP_ID', 'demo-app');
    vi.stubEnv('VITE_ADMIN_SECURITY_AES_KEY', key);
    vi.stubEnv('VITE_ADMIN_SECURITY_AES_IV', iv);
    vi.stubEnv('VITE_ADMIN_SIGNATURE_TYPE', 'A');

    const config = await attachAdminSecurityHeaders({
      headers: {
    ...
    const signText = buildSignString({}, [], traceID, timestamp, 'demo-app');

    expect(resolvePolicyForAlias('auth.logout').requestSign).toEqual([]);
    expect(config.params.sign).toBe(await aesCbcSign(signText, key, iv));
  });
  ...
  it.each(requestOnlyCryptoRoutes)(
    ...
    async (route) => {
      ...
      const responseSign = route.responseSign ?? [];
      const data = Object.fromEntries(
        responseSign.map((field) => [field, `${field}-value`]),
      );
      if (responseSign.length > 0) {
        data.sign = await aesCbcSign(
          buildSignString(data, responseSign, traceID, timestamp, '1'),
          key,
          iv,
        );
      }
      const response = {
        ...
        headers: {
          ...(responseSign.length > 0 ? { 'X-Signature': 'A' } : {}),
          'X-Crypto': 'A',
          'X-Timestamp': timestamp,
          'X-Trace-Id': traceID,
        },
