...
function routeManifestPolicy(
  ...
  return {
    ...
    requestSign: route.requestSign ?? undefined,
    responseCipher: route.responseCipher,
    responseSign: route.responseSign ?? undefined,
  };
}
...
async function verifyResponseSignature(
  ...
  if (policy.responseSign === undefined) {
    return payload;
  }
...
export async function attachAdminSecurityHeaders(config: any) {
  ...
  if (shouldEnableSignature() && policy.requestSign !== undefined) {
