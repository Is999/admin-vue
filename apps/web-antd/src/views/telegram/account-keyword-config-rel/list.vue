...
import {
  fetchAccountKeywordConfigRelList,
  unbindAccountKeywordConfigRel,
    ...
    proxyConfig: {
      ajax: {
        query: async ({ page }: { page: any }, formValues: any) => {
          return await fetchAccountKeywordConfigRelList({
...
function onActionClick(e: OnActionClickParams<AccountKeywordConfigRelItem>) {
  switch (e.code) {
    case 'delete': {
      onDelete(e.row);
      break;
...
async function onStatusChange(
  newStatus: number,
  row: AccountKeywordConfigRelItem,
) {
  const statusLabel: Record<number, string> = { 0: '禁用', 1: '启用' };
  try {
    await confirm(
      `你要将账号 ${row.userID} 的关键词配置 ${row.keywordID} 状态切换为【${statusLabel[newStatus as keyof typeof statusLabel]}】吗？`,
      '切换状态',
    );
    await updateAccountKeywordConfigRelStatus(row.userID, row.keywordID, {
...
function onDelete(row: AccountKeywordConfigRelItem) {
  const hide = message.loading({
    content: `正在解绑账号 ${row.userID} 与关键词配置 ${row.keywordID}...`,
    duration: 0,
    key: 'action_process_msg',
  });
  unbindAccountKeywordConfigRel(row.userID, { keywordIDs: [row.keywordID] })
    .then(() => {
      message.success({
        content: `账号 ${row.userID} 与关键词配置 ${row.keywordID} 解绑成功`,
        key: 'action_process_msg',
      });
      onRefresh();
    })
